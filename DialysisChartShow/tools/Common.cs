...
namespace Dialysis_Chart_Show.tools
{
    ...
    public class JiaMiJieMi
    {
        ...
        private static readonly string Default_AES_Key = "#datacom";
        ...
        public void getRSAKey(string path)
        {
    ...
    public class Common
    {
        ...
        /// <summary>
        /// 回傳管路型号  pif_hpack3是hp3_code, 用來找hp3_name
        /// </summary>
        /// <param name="row"></param>
        /// <returns></returns>
        static public string get_tube_model(DataRow row, string column = "pif_hpack3")
        {
            // 管路型号
            string tube_model = null;
            if (row[column] != null && row[column].ToString().Length != 0)
            {
                tube_model = get_hp3pack_name(row[column].ToString().Trim()); //去除前後空白
            }
            return tube_model;
        }

        /// <summary>
        /// hp3_code like HT000000001
        /// </summary>
        /// <param name="row"></param>
        /// <returns></returns>
        static public string get_hp3pack_code(string code_or_name)
        {
            string column = "hp3_code";
            return get_hp3_value(column, code_or_name);
        }
        ...
        /// <param name="row"></param>
        /// <returns></returns>
        static public string get_hp3pack_name(string code_or_name)
        {
            string column = "hp3_name";
            return get_hp3_value(column, code_or_name);
        }
        ...
        static public void SaveERR(string s1, string s2)
        {
            string dir = "c:\\web.log";
            if (Directory.Exists(dir) == false)
            {
                Directory.CreateDirectory(dir);
            }
            string sFILE = dir + "\\ERR_" + DateTime.Now.ToString("yyyyMMdd") + ".txt";
        ...
        static public void _ErrorMsgShow(string myMessage)
        {
            X.Msg.Show(new MessageBoxConfig
            {
                Title = "错误信息",                
                Message = "<font size='4'>" + myMessage + "</font>",
                Icon = (MessageBox.Icon)Enum.Parse(typeof(MessageBox.Icon), MessageBox.Icon.INFO.ToString()),
                Buttons = MessageBox.Button.OK
            });
        }

        static public void _NotificationShow(string myMessage)
        {
        ...
        static public void SetComboBoxValue(ComboBox cc, string vv, Boolean bb)
        {
            Boolean b = false;
        ...
        //20160606 Alex SetComboBox
        static public void SetComboBox(ComboBox cc, string vv) 
        {
            for (int i = 0; i < cc.Items.Count; i++)
                if ((cc.Items[i].Text == vv) || (cc.Items[i].Value == vv))
                {
        ...
        static public string GetComboBoxText(ComboBox cc)
        {
            if (cc.SelectedItem.Text != null)
            {
        ...
        static public string GetComboBoxValue(ComboBox cc)
        {
            string value = (cc.SelectedItem.Value == null || cc.SelectedItem.Value == "") ? string.Empty : cc.SelectedItem.Value.ToString();
            return value;
        }

        #region 取得星期
        static public string GetWeek(string thisday)
        {
            DateTime date1 = DateTime.Parse(thisday);
            string sWeek = "";
        ...
        public string Get_YMD2(string idate)
        {
            if (idate.Trim() == "")
            {
                return "";
